...
Also, there's a python script at `./adaptive_publisher/send_msgs_test.py` to do some simple manual testing, by sending msgs to the service stream key.
...
But in order to make the automatic docker image build work, you'll need to set the `SIT_PYPI_USER` and `SIT_PYPI_PASS` variables in the Gitlab CI setting page: [Adaptive Publisher CI Setting Page](https://gitlab.insight-centre.org/sit/mps/felipe-phd/adaptive-publisher/settings/ci_cd). (Or make sure the project is set under a Gitlab group that has this setup for all projects in that group).
